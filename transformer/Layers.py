...
class Encoder(nn.Module):
    ...
    def __init__(
            self,
            num_types, d_model, n_layers, n_head, d_k, d_v,
            d_inner, dropout=0.1):
        super().__init__()
    ...
    def forward(self, event_type, event_time, non_pad_mask):
        ...
        enc_output = self.event_type_emb(event_type)

        # 【关键修改】: 不在循环外只加一次，而是改为在每一层循环内注入
        # 移除旧代码: enc_output += tem_enc 

        for enc_layer in self.layer_stack:
            # 【关键修改】: Dense Temporal Injection (对齐 SMURF)
            # 在每一层输入前，都重新注入时间信号，防止深层网络遗忘时间尺度
